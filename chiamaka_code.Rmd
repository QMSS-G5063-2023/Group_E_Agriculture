...
### Tree map of U.S. product export between 2011 - 2021

```{r}

# Aggregate the data
produce_export <- trade_partners %>% 
  select(element,
         item,
         year,
         value) %>% 
...
group_by(item, year) %>% 
  summarise(value = sum(value)) %>% 
filter(year == 2001 | year == 2011 | year == 2021)

produce_export[nrow(produce_export) + 1,] <- list("2001",0,0)
produce_export[nrow(produce_export) + 1,] <- list("2011")
produce_export[nrow(produce_export) + 1,] <- list("2021")

treemap(produce_export,
            index="item",
            vSize="value",
            type="index"
            )
...
plot_ly(
  type = "treemap",
  labels = produce_export$item,
  values = produce_export$value)
