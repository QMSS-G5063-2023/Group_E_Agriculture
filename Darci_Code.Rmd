...
make_production_plot <- function(fruit_veg, y_p,unit, icon_point="data/emojis/Bud.png"){ trade_yield %>% 
  filter(item == fruit_veg, element == y_p, unit == unit ) %>% 
  mutate(image = paste0("data/emojis/",icon_point,".png")) %>% 
  ggplot() + 
    geom_line(aes(x=year, y = value), color="green",size=.5) +
    geom_point(aes(x=year, y = value), color="green", size=.5) +
    geom_image(aes(x=year, y = value,image= image,asp=.5), size=.03) +
    labs(x="Year",y=paste0(y_p," of ",fruit_veg, " (",unit,")")) 
